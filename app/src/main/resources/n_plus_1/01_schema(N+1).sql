...
-- заказы
create table if not exists iso_demo.order
(
id         bigint generated always as identity primary key,
...
create index if not exists idx_orders_user_id on iso_demo.order (user_id);
create index if not exists idx_orders_status  on iso_demo.order (status);

-- строки заказов — именно order_items, как в сущности OrderItem
create table if not exists iso_demo.order_items
(
id         bigint generated always as identity primary key,
order_id   bigint  not null references iso_demo.order(id),
