...
| 8 | **Исходящий pipeline** | `netty-codec-http` | `io.netty.handler.codec.http.HttpServerCodec` (encoder-часть) | `encode()` | HTTP-ответ → байты в сокет |

> Уточнение: в актуальном Netty `HttpResponseEncoder` и `HttpRequestDecoder` объединены внутри `HttpServerCodec` — отдельного класса `HttpServerEncoder` в API нет; ставьте breakpoint именно в `HttpServerCodec` (внутренний класс-энкодер вызывается через `encode()` того же кодека).
