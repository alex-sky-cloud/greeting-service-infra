...
11. Если в задании или в разделе «Что попробовать самостоятельно» считают **попытки оплаты**, в тексте прямо написано, что считать. Одна строка журнала оплат — одна попытка списать деньги по заказу, поэтому «сколько попыток» — это число строк. `id` называют уникальным номером строки. `attempt_no` называют номером попытки у одного заказа и упоминают только если задание про первую, вторую или третью попытку. Запрещено подсказывать, как писать запрос: «не группировать по `id`», «резать / не резать на группы», «`count(*)` и подсчёт по `id` — одно и то же», «`attempt_no` в этом отчёте не нужен».
...
В начале практики темы, где в заданиях статусы названы по-русски, а в базе лежат английские значения, ставится таблица перевода. Для заказов обязательно разделяют `shipped` (отправлено, ещё не вручено) и `delivered` (уже у покупателя). Слово «доставлено» в задании всегда значит `delivered`, не `shipped`.
...
**Тема 08.** Что в неё входит:

- `WITH имя AS (запрос)` — промежуточному запросу дают имя, основной `SELECT` читает это имя как таблицу;
- то же число можно получить подзапросом в `FROM`; `WITH` нужен, чтобы читать шаги сверху вниз;
- два имени подряд: второе читает первое;
- одно имя читают дважды, чтобы не копировать один и тот же подзапрос;
- имя живёт только до конца этого запроса, это не `CREATE TABLE`;
- коротко `WITH RECURSIVE` на ряде чисел; дерева отделов в учебных таблицах нет.
...
| 07 | Условные агрегаты (`FILTER`) | ✅ | ✅ | ✅ | **завершена** |
| 08 | CTE | ✅ | ⬜ | ⬜ | **теория** |
...
*Создано: 2026-08-19. Обновлено: 2026-09-25. Тема 07 завершена, тема 08 — теория.*
