...
- [GROUP BY по нескольким столбцам — составной ключ корзины](#group-by-по-нескольким-столбцам--составной-ключ-корзины)
- [GROUP BY и NULL — отдельная корзина](#group-by-и-null--отдельная-корзина)
...
## GROUP BY по нескольким столбцам — составной ключ корзины

Это отдельное важное правило:

> Если в `GROUP BY` перечислено несколько выражений, PostgreSQL создаёт корзину по **всей комбинации значений**.
...
Ключ корзины состоит сразу из двух частей:

```text
(status, user_id)
```

Строки попадут в одну корзину, только если у них совпадают **и `status`, и `user_id`**.

PostgreSQL не создаёт сначала большую корзину по `status`, а затем вложенные корзины по `user_id`. Логически для каждой строки он сразу определяет один составной ключ:

```text
строка → (значение status, значение user_id) → корзина с таким ключом
```

### Исходные данные для сравнения

Рассмотрим шесть условных заказов:
...
На одних и тех же строках сравним три разных `GROUP BY`.

### Вариант 1 — группировка только по статусу

**Задача:** посчитать количество заказов и общую сумму заказов отдельно для каждого статуса.
...
Ключом корзины служит только `status`:

```text
(pending) → заказы 101, 102, 103
(shipped) → заказы 104, 105, 106
```
...
Все клиенты внутри одного статуса объединены. В результате получилась **одна строка на статус**.

### Вариант 2 — группировка по статусу и клиенту

**Задача:** для каждой пары «статус + клиент» посчитать количество заказов и их общую сумму.
...
Теперь ключ корзины — пара `(status, user_id)`:

```text
(pending, 10) → заказы 101, 102
(pending, 20) → заказ 103
(shipped, 10) → заказ 104
(shipped, 20) → заказы 105, 106
```

**Результат на исходных данных:**

| status | user_id | order_count | total_amount |
|---|---:|---:|---:|
| pending | 10 | 2 | 300 |
| pending | 20 | 1 | 300 |
| shipped | 10 | 1 | 400 |
...
По сравнению с `GROUP BY status` прежние две корзины разделились на четыре. Отчёт стал детальнее: теперь одна строка результата относится не просто к статусу, а к конкретному клиенту внутри конкретного статуса.

### Вариант 3 — в GROUP BY добавлен уникальный id заказа

**Задача:** посмотреть, что произойдёт, если сгруппировать строки одновременно по уникальному номеру заказа и его статусу.
...
```text
(101, pending) → заказ 101
(102, pending) → заказ 102
(103, pending) → заказ 103
(104, shipped) → заказ 104
(105, shipped) → заказ 105
(106, shipped) → заказ 106
```

**Результат на исходных данных:**

| id | status | order_count | total_amount |
|---:|---|---:|---:|
| 101 | pending | 1 | 100 |
| 102 | pending | 1 | 200 |
| 103 | pending | 1 | 300 |
...
`orders.id` — уникальный первичный ключ. Двух строк с одинаковым `id` быть не может. Поэтому каждый заказ получает отдельную корзину, даже если у нескольких заказов одинаковый `status`.

В результате:

- `COUNT(*)` в каждой корзине равен `1`;
- `SUM(amount)` в каждой корзине равна сумме одного заказа;
- суммы всех заказов одного статуса больше не складываются друг с другом.

Добавление `status` к уникальному `id` ничего не объединяет: уникального `id` уже достаточно, чтобы отделить каждую строку от остальных.

### Связь с примером «выручка по категориям»

После соединения `orders → products → product_categories` представим такой промежуточный набор строк:

| order_id | category_code | amount |
|---:|---|---:|
| 1 | electronics | 100 |
| 2 | electronics | 200 |
| 3 | office | 300 |
...
```text
(electronics) → заказы 1 и 2 → SUM = 300
(office)      → заказы 3 и 4 → SUM = 700
```
...
Это подходящая группировка для бизнес-вопроса: **«Какова общая выручка каждой категории?»**

#### GROUP BY по id заказа и категории
...
INNER JOIN products p ON o.product_id = p.id
INNER JOIN product_categories pc ON p.category_id = pc.id
GROUP BY o.id, pc.code;
```

Корзины и результат:

```text
(1, electronics) → только заказ 1 → SUM = 100
(2, electronics) → только заказ 2 → SUM = 200
(3, office)      → только заказ 3 → SUM = 300
(4, office)      → только заказ 4 → SUM = 400
```
...
Два заказа категории `electronics` не объединяются, потому что их полные ключи различаются:

```text
(1, electronics) ≠ (2, electronics)
```

Такой запрос отвечает уже на другой вопрос: **«Какова сумма каждого отдельного заказа и к какой категории он относится?»**

### Порядок столбцов не создаёт вложенность

Для состава корзин эти записи равнозначны:
...
В обоих случаях строки объединяются только при совпадении обоих значений. Перестановка столбцов не превращает один из них в «главную корзину», а другой — во «вложенную».

`GROUP BY` также не сортирует результат. Если порядок строк важен, его нужно отдельно задать через `ORDER BY`.

### Главное правило детализации

Каждый дополнительный столбец в `GROUP BY` уточняет ключ корзины:

```text
GROUP BY status
    одна корзина на статус

GROUP BY status, user_id
    одна корзина на пару «статус + клиент»

GROUP BY status, user_id, id
    одна корзина на тройку «статус + клиент + заказ»
```

Чем больше признаков группировки, тем группы обычно мельче, а строк результата — больше.

Если в `GROUP BY` добавлен уникальный столбец, например `orders.id`, агрегация почти всегда перестаёт сводить несколько заказов в общий итог: каждая исходная строка оказывается в собственной корзине.

### Что важно запомнить для собеседования

1. Несколько столбцов в `GROUP BY` образуют **один составной ключ группы**.
2. Строки объединяются, только когда совпадают **все части** этого ключа.
3. Это не последовательная и не вложенная группировка.
4. Добавление столбца делает отчёт детальнее и может разделить прежние группы.
5. Уникальный `id` в ключе обычно создаёт отдельную группу для каждой строки.
6. Агрегатная функция может быть написана правильно, но отвечать не на тот бизнес-вопрос, если неправильно выбраны границы групп.
...
PostgreSQL не «угадывает» желаемый уровень отчёта. Разработчик сам определяет его списком выражений в `GROUP BY`.

---

## GROUP BY и NULL — отдельная корзина

Это правило часто встречается после `LEFT JOIN`, поэтому его нужно знать отдельно.

> Для `GROUP BY` все значения `NULL` считаются **одним и тем же ключом группы**. Они попадают в **одну** корзину.

Это не противоречит правилу `NULL = NULL → UNKNOWN`. Для обычного сравнения `=` два `NULL` не равны. Для группировки PostgreSQL использует другое правило: «значения неразличимы» (`not distinct`). Два `NULL` неразличимы, поэтому их складывают вместе.

### Официальное правило PostgreSQL

**Утверждение:** `GROUP BY` собирает в одну строку все выбранные строки с одинаковыми значениями группируемых выражений. Группировка без агрегатов даёт тот же набор, что и `DISTINCT`. Для `DISTINCT` значения `NULL` считаются одинаковыми.
...
> Группировка без агрегатных выражений фактически вычисляет набор различных значений столбца. Того же результата можно добиться через `DISTINCT`.

**Источник:** https://www.postgresql.org/docs/current/queries-select-lists.html
...
> Две строки считаются различными, если они отличаются хотя бы в одном значении столбца. В этом сравнении значения `NULL` считаются одинаковыми.

Отсюда следует: все строки с `NULL` в столбце группировки попадают в одну группу.

### Исходные данные

После `LEFT JOIN` категория у заказа может быть пустой:
...
Строка 3 — заказ без товара. После `LEFT JOIN` поле `pc.name` заполнилось `NULL`. Строка 5 — ещё один такой же заказ.

### Как GROUP BY раскладывает корзины
...
Заказы 3 и 5 не «теряются» и не образуют две разные корзины. У них одинаковый ключ группы — `NULL`.

### Почему это не то же самое, что `NULL = NULL`
...
`WHERE` оставляет только `TRUE`, поэтому такие проверки отбрасывают строку.

`GROUP BY` не спрашивает «равны ли значения по `=`». Он спрашивает: «различимы ли ключи». Два `NULL` неразличимы, поэтому они идут в одну корзину.

### COUNT(*) и COUNT(столбец) в корзине NULL

В той же корзине `(NULL)`:

```text
COUNT(*)              = 2   — считает строки корзины
COUNT(category_name)  = 0   — считает только не-NULL значения category_name
```

`COUNT(*)` отвечает: «сколько заказов попало в эту корзину».  
`COUNT(category_name)` отвечает: «у скольких строк этой корзины название категории заполнено». В корзине `NULL` заполненных названий нет, поэтому результат `0`.
...
```text
1. LEFT JOIN не нашёл товар или категорию
2. В готовой строке pc.name = NULL
3. GROUP BY pc.name кладёт все такие строки в одну корзину
4. COUNT(*) считает эти заказы как отдельную группу «без категории»
```

Это не особый фокус `LEFT JOIN`. `GROUP BY` одинаково обрабатывает любой `NULL`: и исходный из таблицы, и подставленный `LEFT JOIN`.

### Что важно запомнить для собеседования

1. `GROUP BY` создаёт отдельную корзину для `NULL`.
2. Все `NULL` в ключе группы попадают в **одну** корзину, а не в разные.
3. Это правило группировки, а не правило оператора `=`.
4. После `LEFT JOIN` заказы без товара обычно видны как группа с `category_name = NULL`.
5. `COUNT(*)` в этой группе считает заказы; `COUNT(pc.name)` даст `0`.
