...
**Задача:** оператор поддержки открыл карточку **Ann Smith** (`users.id = 1`) и должен видеть её заказы **вместе с именем** — без отдельного поиска в таблице `users`.

Вывести **3** заказа (для наглядности) со столбцами:

| Столбец | Откуда |
|---|---|
| id заказа | `orders.id` |
| имя клиента | `users.full_name` |
| сумма | `orders.amount` |
| статус | `orders.status` |

Сортировка по id заказа (по возрастанию). Только **INNER JOIN** — пары «заказ + клиент».
...
### Пример 4 — на маленьких таблицах (механика LEFT JOIN)

**Задача:** маркетинг строит отчёт «**каждый** клиент и его заказы».  
Клиент **Carol** зарегистрирована, но **ни разу не заказала** — её строка **обязана** быть в результате (столбцы заказа пустые).

Нужны столбцы: **имя клиента**, **id заказа**, **сумма заказа**.
...
### Пример 6 — сравнение клиента с заказами и без

**Задача:** аналитик сравнивает **двух** учебных клиентов в **одном** запросе:

| email | Ситуация |
|---|---|
| `join-demo-no-orders-1@example.com` | зарегистрирован, **0** заказов |
| `join-demo-single-order@example.com` | есть **несколько** учебных заказов |

Нужны столбцы: **email**, **имя**, **id заказа**, **сумма заказа**.  
Клиент без заказов **не должен пропасть** — у него одна строка с пустым id заказа.
...
**Задача:** для **четырёх** учебных адресов `join-demo-no-orders-1@example.com` … `join-demo-no-orders-4@example.com` сравнить поведение **INNER JOIN** и **LEFT JOIN**.

Фильтр: `WHERE u.email LIKE 'join-demo-no-orders-%'`.  
Столбцы: **email** и **id заказа**.

**Ожидание:** INNER — **0 строк**; LEFT — **4 строки** с пустым id заказа.
...
**Задача:** складу нужны **доставленные** заказы клиента **Ann Smith** (`users.id = 1`) вместе с **email** для SMS-опроса.

Столбцы: **id заказа**, **email клиента**, **сумма**.  
Фильтр: `orders.status = 'delivered'` и `users.id = 1`.  
Сортировка по id заказа; в примере — до **3** строк.
...
**Результат на вашей базе (фрагмент — у Ann Smith только 2 заказа со статусом delivered):**
