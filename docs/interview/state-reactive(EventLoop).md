...
> 
> - Он **переключает стрелки**, **принимает короткие команды**, выполняет и сразу берёт следующую.
> 
...
```
 ...
 - Ключевыми **компонентами** архитектуры являются:
   - **Channel** — абстракция сетевого соединения (например, TCP-сокета).
   - **EventLoop** — поток, который управляет обработкой событий для конкретного соединения.(имеет бесконечный цикл и выполняет задачи типа **Runnable** из "Queue\<Runnable\>")
   - **Pipeline** — цепочка обработчиков (**ChannelHandler**), через которую проходят данные. 
     - Каждый обработчик (**ChannelHandler**) выполняет определённую часть логики: декодирование входящих данных, кодирование исходящих, обработку бизнес-логики, шифрование и т.д..

**Netty** использует собственный буферный API (**ByteBuf**), который позволяет реализовывать «нулевое копирование» (**zero-copy**) — передачу данных без лишних копий в памяти.


Представь, что у тебя есть **одна и та же коробка с деталями**, и ты хочешь показать её двум людям.

Обычный способ — **переложить детали в две новые коробки**. Это долго и требует лишней работы.

**Zero-copy** — это когда ты **не перекладываешь детали**, а просто даёшь второму человеку **посмотреть на ту же самую коробку**.
 - Детали остаются на месте, а ты экономишь время и силы.

В Netty смысл именно такой: **не делать лишнюю перекладку байтов из одного буфера в другой**, если можно использовать уже имеющиеся байты напрямую.

### **Zero-copy** - как выглядит:

 - данные уже есть в памяти;
 - Netty не делает их новую копию;
 - вместо этого он работает с теми же данными, как с одной коробкой, к которой дают доступ разными способами.
...
#### Пример: два пользователя используют один поток для некоторого EventLoop
