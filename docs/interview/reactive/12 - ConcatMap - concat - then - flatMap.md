...
### Объединение готовых потоков

| Оператор | Назначение |
| :-- | :-- |
| `Flux.concat(a, b)` / `a.concatWith(b)` | Склеить **элементы** двух потоков. Ошибка в первом — второй не стартует. |


***

### Обработка каждого элемента Flux

| Оператор | Порядок | Конкурентность |
| :-- | :-- | :-- |
| `concatMap(fn)` | Строгий (след. после завершения пред.) | 1 за раз |
| `flatMap(fn)` | Негарантирован | Параллельно (merge) |
| `flatMap(fn, N)` | Негарантирован | До N параллельно |
...
| Оператор | Когда |
| :-- | :-- |
| `then(next)` | Значение не нужно, важен факт успеха |
| `flatMap(v → next(v))` | Следующий шаг зависит от значения |
| `filter(p).switchIfEmpty(fb)` | Проверить значение — решить: идти/прерваться |


***

### Ловушка eager-аргументов

```java
// ❌ Вызовется при СБОРКЕ цепочки
check.then(paymentService.charge(...))

// ✅ Вызовется ПОСЛЕ onComplete check
check.then(Mono.defer(() -> paymentService.charge(...)))
```

*Если метод делает побочные эффекты в теле — оборачивай в `Mono.defer`.*

***

### Не путать

| Оператор | Реагирует на | Что делает |
| :-- | :-- | :-- |
| `concatWith` | `onNext` (элементы) | Складывает элементы в один Flux |
| `then` / `flatMap` | `onComplete` / `onNext` | Передаёт управление между шагами |
...
requests
    .concatMap(req -> 
        ensureSufficientBalance(req.accountId(), req.amount())
            .then(Mono.defer(() -> paymentGateway.charge(req)))
            .then(Mono.defer(() -> ledger.record(req)))
            .thenReturn(new Result(req.id(), "OK"))
    )
    .subscribe();
```

- `concatMap` — заявки по очереди
- `then` — шаги одной заявки строго последовательно
- `Mono.defer` — защита от eager-эффектов
