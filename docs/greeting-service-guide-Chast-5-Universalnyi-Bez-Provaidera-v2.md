...
>   - В этой версии внутри каждого блока кода добавлена пустая строка сразу после открывающей строки блока (после ```` ```bash ```` / ```` ```yaml ```` и т.п.), перед первой содержательной строкой кода. 
>   - Это применено системно во всех блоках документа.

> **Суть документа не изменилась.** 
>  - Отвечаю на вопрос про плавающий IP (Раздел 0) и полностью убираю зависимость от managed-сервисов провайдера: Kubernetes-кластер разворачивается вручную через `k3s` на голых виртуальных серверах, PostgreSQL — самостоятельно как `StatefulSet`. 
>  - Единственное, что заказывается у провайдера, — обычные виртуальные серверы (VPS) и публичные/плавающие IP-адреса.
...
**Плавающий IP** (floating IP) — это **статичный публичный адрес**, который сам по себе **не меняется**. 
 - Его особенность не в том, что он «плавает» во времени сам, а в том, что вы можете **вручную** пере-привязать его от одного сервера к другому, не меняя сам IP-адрес:
...
**RU**: «Плавающий IP — это публичный, статический IP-адрес, который можно динамически переназначать между разными устройствами в вашей сети, что обеспечивает высокую доступность, отказоустойчивость и failover для приложений и сервисов в облаке... 
   
  - **Плавающий IP** — это статический публичный адрес, который можно переназначить сетевому устройству без необходимости менять местоположение самого устройства.»
...
### Практический смысл для вашего проекта
  ...
  - Он **не меняется автоматически** при перезапуске или пере-сборке сервера — в этом и есть его ценность по сравнению с обычным публичным IP, который у многих провайдеров может смениться после пересоздания VPS.
...
> «Чтобы заказать новый IP:
> 
> 1. Войдите в панель управления Рег.облака. 
> 2. Перейдите в раздел Мои ресурсы → Плавающие IP. 
> 3. Кликните Новый ресурс и выберите Плавающий IP. 
> 4. Выберите город размещения IP и нажмите Добавить плавающий IP...
> 
> Чтобы привязать IP-адрес: ...выберите Привязать к серверу.»
...
> «Плавающие IP-адреса не назначаются виртуальным машинам по умолчанию.
> 
> Пользователи облака должны явным образом "взять" их из пула, настроенного администратором облака.»
...
- Больше **ничего** заказывать у провайдера не требуется. 
- VPC/приватная сеть используется только если провайдер предоставляет её бесплатно и автоматически (тогда это просто удобство, не обязательный шаг) — сервера прекрасно взаимодействуют и через публичные IP с ограничением доступа через firewall.
